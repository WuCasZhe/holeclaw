...
class ArchiveTests(unittest.TestCase):
    ...
    def test_capped_resume_preserves_existing_comments(self):
        ...
        self.store.ingest_comments(dict(self.payload, comments=comments, comment_page=120))
        resume = self.store.prepare_posts([self.post], 'run-1')['1']
        self.assertEqual(len(resume['saved_comment_ids']), 1000)
        self.assertEqual(self.store.summary()['comments'], 1200)
        self.store.ingest_comments(dict(self.payload, comments=[], comment_page=120, complete=True))
        self.assertTrue(self.store.resume_comments('1', 'run-2', reply=0)['complete'])
        self.assertEqual(self.store.summary()['comments'], 1200)
    ...
    def test_run_summary_excludes_unselected_historical_archive_rows(self):
        ...
        self.assertEqual(summary, dict(posts=1, comments=0, completed_comment_posts=1,
                                       incomplete_comment_posts=0, oldest_archived_post_timestamp=150))
