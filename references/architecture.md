...
图片状态按批读取，文件存在性和大小的检查结果在一次完整检查中跨批复用，最多保留 4096 项；不同检查之间不复用，以便后续发现文件被删除或截断。归档摘要默认将 `cache_integrity` 标为 `not_checked`；`archive --verify-cache` 显式执行全库校验，避免每次复用都扫描整个数据库。
...
新图片客户端向 `/media` 上传 Blob，`X-Holeclaw-Message` 携带 URI 编码的 v3 JSON 元数据，沿用原随机令牌和 Origin 校验。服务端最多接收两个并行上传，流式写入独立临时文件并计算 SHA-256、同步文件；这些工作在 sink 锁外完成。发布前将实际内容哈希加入回执摘要，在锁内检查重复请求、原子重命名并提交元数据。已存在回执的请求走只读流式哈希校验，避免重复落盘；校验期间回执过期会拒绝请求，不会将未落盘内容发布。上传失败清理临时文件，旧 Base64 回调继续兼容。
...
评论 UPSERT 只在正文、时间、署名或引用变化时更新行。评论行 `observed_at` 保留该内容版本的观察时间；`comment_scans.observed_at` 继续记录最新扫描，游标更新与评论变化同事务提交。

新建的完整缓存任务先在同一 SQLite 事务的源库快照内验证评论和图片是否齐备；全部齐备则分批登记成员并直接完成，不创建 `archive_candidates` 正文副本。部分缺失时返回原快照流程，已有未完成检查点仍按持久化候选页续传。SQLite 提交和 JSON 保存仍是两个独立步骤，中断后可以重做本地登记。

`archive_totals` 保存帖子、评论和完成扫描总数，首次升级回填。插入、删除及扫描完成状态变化触发器在原事务中维护计数，回滚同步撤销；摘要只读取三条计数，窗口统计仍按本次成员计算。

检查点写入保留原子替换及文件权限，改用紧凑 JSON。进程内最多记住 128 个路径的内容摘要和文件状态，完全未变化时跳过写入；失败的保存不更新记录，删除或外部修改后重新写入。缓存不保留整份检查点正文。

`tests/test_io.py` 覆盖重复评论写入量、增量计数回填和回滚、完整缓存快速路径及部分缓存快照、图片重试校验与回执过期、跨批文件检查以及检查点外部修改和写入失败。
