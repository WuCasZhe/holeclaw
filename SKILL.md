...
Store checkpoint schema v4 under `output/playwright/holeclaw-checkpoints-v4/` and cache schema v5 at `output/playwright/holeclaw-cache-v5.sqlite3`, both with mode `0600`. Older default files remain untouched and are not migrated; an explicitly supplied incompatible cache or checkpoint must fail and ask for a new path. The cache is independent of the report threshold: reuse complete coverage for different thresholds, AND/OR modes, or shorter ranges, and scan only the new head when an older coverage interval contains the requested start. Within schema v5, comment-only coverage with incomplete favorite data remains reusable for comment-only reports, but favorite-filtered reports must rescan that coverage once to populate `likenum`. If a list favorite count is missing or invalid, fetch only that post's detail once. If the detail also lacks a usable count, record the PID as explicitly unavailable, keep the rest of the favorite coverage reusable, and mention it in the report. Use `--fresh` only when the user explicitly requests a network refresh that ignores reusable coverage.
